...
RUN apt-get update && \
    ...
    wget http://archive.ubuntu.com/ubuntu/pool/universe/m/mysql-5.5/libmysqlclient16_5.5.62-0ubuntu0.14.04.1_amd64.deb && \
    dpkg -i libmysqlclient16_5.5.62-0ubuntu0.14.04.1_amd64.deb || apt-get install -f -y && \
    rm -f libmysqlclient16_5.5.62-0ubuntu0.14.04.1_amd64.deb && \
    if [ ! -f /usr/lib/libmysqlclient.so.16 ]; then \
        cp /usr/lib/x86_64-linux-gnu/libmysqlclient.so.16* /usr/lib/libmysqlclient.so.16 2>/dev/null || \
        ln -sf /usr/lib/x86_64-linux-gnu/libmysqlclient.so.16 /usr/lib/libmysqlclient.so.16; \
    fi && \
